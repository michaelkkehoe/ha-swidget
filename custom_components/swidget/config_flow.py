...
from pyswidget.discover import Discover
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def __init__(self) -> None:
        ...
        self._discovered_devices: dict[str, SwidgetDevice] = {}
        self._discovered_device: SwidgetDevice | None = None
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                device = await self._async_try_connect(host, user_input["secret"], raise_on_progress=False)
            except SwidgetException:
        ...
        return self.async_show_form(
            step_id="user",
            data_schema=vol.Schema({vol.Required(CONF_HOST): str, vol.Required("secret"): str}),
            errors=errors,
        )
    ...
    async def _async_try_connect(
        self, host: str, secret: str, raise_on_progress: bool = True
    ) -> SwidgetDevice:
        """Try to connect."""
        self._async_abort_entries_match({CONF_HOST: host})
        device: SwidgetDevice = await SwidgetDevice(host, secret).ping()
        await self.async_set_unique_id(
            dr.format_mac(device.mac_address), raise_on_progress=raise_on_progress
        )
        return device
